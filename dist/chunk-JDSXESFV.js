...
// src/supabase/services/storage.service.ts
var StorageService = class _StorageService {
  ...
  uploadFile(bucket, name, file) {
    return __async(this, null, function* () {
      const { data, error } = yield this.supabase.storage.from(bucket).upload(name, file);
      if (error)
  ...
  downloadFile(bucket, path) {
    return __async(this, null, function* () {
      const { data, error } = yield this.supabase.storage.from(bucket).download(path);
      if (error)
        throw new Error(`Download Error: ${error.message}`);
      return data;
    });
  }
  // READ - List files in the bucket
  listFiles(bucket, folderPath = "") {
    return __async(this, null, function* () {
      const { data, error } = yield this.supabase.storage.from(bucket).list(folderPath);
      if (error)
  ...
  moveFile(bucket, oldPath, newPath) {
    return __async(this, null, function* () {
      const { data, error } = yield this.supabase.storage.from(bucket).move(oldPath, newPath);
      if (error)
  ...
  deleteFile(bucket, paths) {
    return __async(this, null, function* () {
      const { data, error } = yield this.supabase.storage.from(bucket).remove(paths);
      if (error)
  ...
  static \u0275fac = function StorageService_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || _StorageService)(\u0275\u0275inject(SupabaseContext));
  };
...
@angular/forms/fesm2022/forms.mjs:
   ...
   *)
*/
//# sourceMappingURL=chunk-JDSXESFV.js.map
