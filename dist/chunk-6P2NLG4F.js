...
// src/app/shared/app-services/excel.service.ts
var EXCEL_TYPE = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;charset=UTF-8";
var ExcelService = class {
  static exportArrayToExcel(normalizedData, fileName) {
    const worksheet = utils.json_to_sheet(normalizedData);
    const colWidths = [];
    Object.keys(normalizedData[0]).forEach((column) => {
      console.log(column);
  ...
  static getEmployeeData(file, employees) {
    return __async(this, null, function* () {
      return new Promise((resolve, reject) => {
        const reader = new FileReader();
        let convertExcelDate = (value) => {
        ...
        reader.onload = (e) => {
          try {
            ...
            const workbook = readSync(binaryData, { type: "binary" });
            const sheetName = workbook.SheetNames[0];
            const sheetData = workbook.Sheets[sheetName];
            const rawData = utils.sheet_to_json(sheetData);
            const normalizeEmail = (email) => String(email || "").trim().replace(/[\u200E\u200F\u202A-\u202E]/g, "").toLowerCase();
            const jsonData = rawData.reduce((mappedEmployees, row) => {
              const isValidEmail = (email) => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email);
              if (!isValidEmail(row["email"])) {
                return mappedEmployees;
              }
              const employee = mappedEmployees.find((emp) => normalizeEmail(emp.email) === normalizeEmail(row["email"]));
              const child = {};
              if (row["children_name"])
                child.name = row["children_name"];
              if (row["children_birth_date"])
                child.birth_date = convertExcelDate(row["children_birth_date"]);
              if (row["children_gender"])
                child.gender = row["children_gender"];
              if (employee) {
                if (child.name && child.birth_date) {
                  employee.childrens.push(child);
                }
              } else {
                const newEmployee = {};
                const email = normalizeEmail(row["email"]);
                const existEmployee = employees.find((emp) => emp.email && normalizeEmail(emp.email.toLowerCase()) === email);
                if (!existEmployee) {
                  return mappedEmployees;
                }
                newEmployee.id = existEmployee.id;
                if (row["gender"])
                  newEmployee.gender = row["gender"];
                if (row["date_of_birthday"])
                  newEmployee.date_of_birthday = convertExcelDate(row["date_of_birthday"]);
                if (row["address"])
                  newEmployee.address = row["address"];
                if (row["marital_status"])
                  newEmployee.marital_status = row["marital_status"];
                if (row["partner_display_name"])
                  newEmployee.partner_display_name = row["partner_display_name"];
                if (row["hebrew_display_name"])
                  newEmployee.hebrew_display_name = row["hebrew_display_name"];
                if (row["card_id"])
                  newEmployee.card_id = row["card_id"];
                if (row["phone"])
                  newEmployee.phone = row["phone"];
                if (row["work_phone"])
                  newEmployee.work_phone = row["work_phone"];
                if (row["company"])
                  newEmployee.company = row["company"];
                if (row["department"])
                  newEmployee.department = row["department"];
                if (row["joining_date"])
                  newEmployee.joining_date = convertExcelDate(row["joining_date"]);
                if (row["job_position"])
                  newEmployee.job_position = row["job_position"];
                if (row["division"])
                  newEmployee.division = row["division"];
                if (row["role"])
                  newEmployee.role = row["role"];
                if (row["is_active"])
                  newEmployee.is_active = row["is_active"];
                if (row["ad_id"])
                  newEmployee.ad_id = row["ad_id"];
                if (row["display_name"])
                  newEmployee.display_name = row["display_name"];
                if (row["email"])
                  newEmployee.email = email;
                if (row["manager_email"]) {
                  let manager = employees.find((x) => x.email && normalizeEmail(x.email.toLowerCase()) == normalizeEmail(row["manager_email"].toLowerCase()));
                  if (manager) {
                    newEmployee.reporting_manager_id = manager.id;
                    manager.role = AppConstants.ROLE_MANAGER;
                  } else {
                    console.log(row["manager_email"]);
                  }
                }
                if (child.name && child.birth_date) {
                  newEmployee.childrens = [child];
                } else {
                  newEmployee.childrens = [];
                }
                mappedEmployees.push(newEmployee);
              }
              return mappedEmployees;
            }, []);
            resolve(jsonData);
  ...
  static getObjectiveTemplatesData(file) {
    return __async(this, null, function* () {
      return new Promise((resolve, reject) => {
        const reader = new FileReader();
        reader.onload = (e) => {
          try {
            ...
            const rawData = utils.sheet_to_json(sheetData);
            const jsonData = rawData.reduce((mappedEmployees, row) => {
              const isValidType = (type) => type == "quantity" || type == "quality";
              if (!row["type"] || !isValidType(row["type"])) {
                throw new Error("For one or more templates type not valid");
              }
              if (!row["criteria"]) {
                throw new Error("Criteria required for all templates");
              }
              const newTemplate = {};
              if (row["comments"])
                newTemplate.comments = row["comments"];
              if (row["criteria"])
                newTemplate.criteria = row["criteria"];
              if (row["load_percentage"])
                newTemplate.load_percentage = row["load_percentage"];
              if (row["type"])
                newTemplate.type = row["type"];
              if (row["unit_of_measurement"])
                newTemplate.unit_of_measurement = row["unit_of_measurement"];
              if (row["weight"])
                newTemplate.weight = row["weight"];
              if (row["weighted_score"])
                newTemplate.weighted_score = row["weighted_score"];
              if (row["yearly_goal"])
                newTemplate.yearly_goal = row["yearly_goal"];
              if (row["yearly_performance"])
                newTemplate.yearly_performance = row["yearly_performance"];
              mappedEmployees.push(newTemplate);
              return mappedEmployees;
            }, []);
            resolve(jsonData);
          } catch (error) {
  ...
  static getFeedbackTemplatesData(file) {
    return __async(this, null, function* () {
      return new Promise((resolve, reject) => {
        const reader = new FileReader();
        reader.onload = (e) => {
      ...
      return new Promise((resolve, reject) => {
        ...
        reader.onload = (e) => {
          try {
            ...
            const sheetData = workbook.Sheets[sheetName];
            const rawData = utils.sheet_to_json(sheetData);
            const jsonData = rawData.reduce((criterias, row) => {
              const newTemplate = {};
              if (row["criteria"])
                newTemplate.value = row["criteria"];
              criterias.push(newTemplate);
              return criterias;
            }, []);
            resolve(jsonData);
          } catch (error) {
...
*/
//# sourceMappingURL=chunk-6P2NLG4F.js.map
